...
def plot_local_step_forecast(cfg, X, y, forecasts, modes, dst_dir, sample_id, epoch, confidence_levels, ade, fde, src):
    """Plot a 3D forecast example in local step-displacement coordinates.
    """
    ...
    ax.set_xlabel('local sideways [m]')
    ax.set_ylabel('local forward [m]')
    ax.set_zlabel('delta z [m]')
    ax.legend()
    ...
    ax_xy.set_xlabel('local sideways [m]')
    ax_xy.set_ylabel('local forward [m]')
    ax_xy.set_title('local horizontal step')
    ax_xy.grid(True)
    ax_xy.axis('equal')
    
    t_in = np.arange(input_track.shape[0])
    t_out = np.arange(input_track.shape[0], input_track.shape[0] + target_track.shape[0])
    ax_z.plot(t_in, input_track[:,2], color='tab:blue', marker='o', label='input delta z')
    ax_z.plot(t_out, target_track[:,2], color='black', marker='o', label='target delta z')
    ax_z.plot(t_out, modes[:,2], color='tab:orange', marker='o', label='mode delta z')
    ax_z.set_xlabel('timestep')
    ax_z.set_ylabel('delta z [m]')
    ax_z.set_title('vertical step over time')
    ax_z.grid(True)
    ax_z.legend()
    ...
    ax_xz.set_xlabel('local sideways [m]')
    ax_xz.set_ylabel('delta z [m]')
    ax_xz.set_title('sideways-delta z view')
    ax_xz.grid(True)
