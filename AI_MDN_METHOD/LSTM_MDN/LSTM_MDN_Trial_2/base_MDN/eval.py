...
from vis import plot_aee_over_time, plot_local_step_forecast, plot_world_forecast
from vis import plot_reliability_calibration, plot_sharpness_over_time
from utils.helper import local_steps_to_world
...
class MDN_Forecaster:
    ...
    def __init__(self, cfg, model, data_loader, type, logger, device='cpu'):
        ...
        if type == 'eval':
            ...
            self.plot_local_dst_dir = cfg.eval_local_examples_path
            self.plot_world_dst_dir = cfg.eval_world_examples_path
            self.data_X, self.data_y, self.data_ref, self.data_rot, self.data_src, self.data_input_world, self.data_target_world = data_loader.get_eval_data()
        ...
        # testing a trained model with test data
            ...
            self.plot_local_dst_dir = cfg.test_local_examples_path
            self.plot_world_dst_dir = cfg.test_world_examples_path
            self.data_X, self.data_y, self.data_ref, self.data_rot, self.data_src, self.data_input_world, self.data_target_world = data_loader.get_test_data()
    ...
    def evaluate(self, epoch=None):
        ...
        # create mesh grid for 3D local displacement coordinates
        grid = None
    ...
    def save_examples(self, mesh_range_x, mesh_range_y, mesh_resolution, confidence_levels, n_samples, plot_local, plot_map, epoch=None):
        ...
        with torch.no_grad():
            ...
            for p in range(0, len(self.data_X), self.plot_step):
                ...
                src = self.data_src[p]
                input_world = self.data_input_world[p]
                target_world = self.data_target_world[p]
                ...
                if plot_local:
                    ...
                    plot_local_step_forecast(
                        cfg=self.cfg,
                        X=input.cpu().numpy(),
                        y=filtered_target.cpu().numpy(),
                        forecasts=None,
                        modes=modes.cpu().numpy(),
                        dst_dir=self.plot_local_dst_dir,
                        sample_id=p,
                        epoch=epoch,
                ...
                if plot_map:
                    ...
                    filtered_target_world = target_world[self.test_params['test_horizons']]
                    modes_world = local_steps_to_world(
                        local_steps=modes.cpu().numpy(),
                        start_position=reference_pos,
                        start_heading_angle=rotation_angle
                    )
                    
                    plot_world_forecast(
                        cfg=self.cfg,
                        input_world=input_world,
                        target_world=filtered_target_world,
